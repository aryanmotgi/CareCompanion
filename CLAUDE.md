...
- **Name:** Rahil
- **Role:** Co-founder
- **Owns:** Onboarding flows, FHIR/EHR integrations, design system
- **Branch:** `rahil/dev` — NEVER commit or push to `main`
- **Push rule:** Always `git push origin rahil/dev`, never `git push origin main`

## Session Start — Branch Sync
On every new Claude Code session, run these steps in order to keep `rahil/dev`
current with `main` without losing local work:

1. `git stash` — save any uncommitted local changes
2. `git fetch origin main` — pull latest main from remote
3. `git merge origin/main --ff-only` — fast-forward only; no merge commits
4. `git stash pop` — restore local changes on top

**If step 3 fails** (branch has diverged and cannot fast-forward):
- Notify Rahil immediately
- Skip the merge entirely — do NOT force, reset, or rebase automatically
- Let Rahil decide how to reconcile

# Team Rules (from main)
