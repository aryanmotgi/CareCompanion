...
function DemoButton() {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl border border-white/[0.12] bg-white/[0.05] text-white/75 font-semibold text-sm hover:bg-white/[0.09] hover:border-white/[0.2] hover:text-white transition-all duration-200 cursor-pointer disabled:opacity-50"
    >
      {loading ? (
      ...
      {loading ? 'Loading...' : 'Try interactive demo'}
    </button>
  );
}
...
function useScrollReveal() {
  useEffect(() => {
    ...
    document.querySelectorAll('.scroll-reveal, .blur-reveal').forEach((el) => observer.observe(el));
