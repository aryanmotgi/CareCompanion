...
Be warm and concise. Never say you are in demo mode or mention limitations.`,
  ...
  const [
    ...
    profile?.id ? db.select().from(medications).where(eq(medications.careProfileId, profile.id)) : Promise.resolve([]),
    profile?.id ? db.select().from(doctors).where(eq(doctors.careProfileId, profile.id)) : Promise.resolve([]),
    profile?.id ? db.select().from(appointments).where(eq(appointments.careProfileId, profile.id)) : Promise.resolve([]),
