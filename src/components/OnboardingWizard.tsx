      ...
      {step === 6 && (
        <div key={animKey} className="space-y-8 py-4" style={{ animation: 'celebratePop 0.5s ease-out' }}>
          ...
          <div className="flex justify-center gap-2">
            {['#A78BFA', '#60A5FA', '#34D399', '#F472B6', '#FB923C'].map((c, i) => (
              <div key={i} className="w-2 h-2 rounded-full" style={{ background: c, animation: `confettiFade 0.4s ease-out ${0.3 + i * 0.1}s both` }} />
            ))}
          </div>
