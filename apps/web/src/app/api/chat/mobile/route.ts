...
export async function POST(req: Request) {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      meds.length > 0 ? `Medications: ${(meds as { name: string }[]).map(m => m.name).join(', ')}` : '',
      appts.length > 0 ? `Upcoming appointments: ${(appts as { purpose?: string | null; specialty?: string | null }[]).map(a => a.purpose || a.specialty || 'Appointment').join(', ')}` : '',
