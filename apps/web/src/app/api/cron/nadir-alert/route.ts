/**
 * Cron: Nadir Week Kickoff Alert.
 *
 * Runs daily. For every active treatment cycle where today is day 7
 * (the first day of the nadir window), pushes a family-wide alert to
 * EVERY member of the patient's care group — patient + all caregivers,
 * not just the primary. Once per cycle per user (deduped via
 * notificationDeliveries category='nadir_kickoff' since cycle.startDate).
 */
...
export async function GET(req: Request) {
  ...
  try {
    ...
    for (const cycle of activeCycles) {
      ...
      try {
        ...
        if (!patientMembership) {
          logger.warn('nadir_alert_no_care_group', {
        ...
        const existingDeliveries = await db
          ...
          .where(and(
            ...
            eq(notificationDeliveries.category, 'nadir_kickoff'),
        ...
        for (const userId of recipientIds) {
          try {
            ...
            logger.error('nadir_alert_user_failed', { userId, error: msg });
          }
        }
      } catch (cycleErr) {
        const msg = cycleErr instanceof Error ? cycleErr.message : String(cycleErr);
        errors.push(`cycle ${cycle.cycleId}: ${msg}`);
        logger.error('nadir_alert_cycle_failed', { cycleId: cycle.cycleId, error: msg });
      }
    }

    logger.info('nadir_alert_done', {
    ...
    logger.error('nadir_alert_failed', { error: msg });
